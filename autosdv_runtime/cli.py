#!/usr/bin/env python3
"""
AutoSDV Unified Command Line Interface
Provides a simple, unified interface for managing AutoSDV in production
"""
...
class AutoSDV:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.launch_script = self.package_share_dir / 'scripts' / 'autosdv-launch.sh'
        except RuntimeError:
            # Fallback: try to find it relative to the workspace
            install_dir = self.workspace_dir / 'install' / 'autosdv_runtime'
            if install_dir.exists():
                self.package_share_dir = install_dir / 'share' / 'autosdv_runtime'
                self.launch_script = self.package_share_dir / 'scripts' / 'autosdv-launch.sh'
            else:
    ...
    def start(self, args):
        ...
        try:
            ...
            if result.stdout.strip() == 'active':
                ...
                self.print_info("View logs: autosdv status")
                return True
            else:
    ...
    def status(self, args):
        ...
        if service_status == 'active':
            # Get runtime
            result = subprocess.run(
                ['systemctl', '--user', 'show', 'autosdv', '--property=ActiveEnterTimestamp'],
                capture_output=True, text=True
            )
            if '=' in result.stdout:
        ...
        if service_status == 'active':
            print("  autosdv stop     - Stop the system")
            print("  autosdv restart  - Restart the system")
            print("  autosdv monitor  - Open web monitor")
        else:
...
def main():
    """Main entry point for autosdv command"""
    parser = argparse.ArgumentParser(
        description='AutoSDV - Unified management interface for autonomous vehicle system',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  autosdv install   # Install systemd service
  autosdv start     # Start the system
  autosdv status    # Check system status
  autosdv monitor   # Open web monitor in browser
  autosdv stop      # Stop the system
