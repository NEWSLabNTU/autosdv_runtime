...
class SystemDInstaller:
    def __init__(self):
        ...
        try:
            ...
            logger.error(
                "Could not locate the AutoSDV workspace. The units need its "
                "path; set AUTOSDV_WORKSPACE or run from inside the workspace."
            )
            sys.exit(1)
    ...
    def render_unit(self, text: str) -> str:
        ...
        The units used to hardcode %h/AutoSDV, which is wrong for every
        ...
        """
        rendered = text.replace("@AUTOSDV_WORKSPACE@", str(self.workspace_dir))
